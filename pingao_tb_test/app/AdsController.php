...
class AdsController extends CommonController
{
    ...
    function __construct()
    {
        ...
        if(strpos(server('http_referer'), 'https://') !== false){
            ...
            $this->wechat_tz_domain = 'http://ptd.5177jy.com'; //点击微信
    ...
    public function adsAction($ad_pos, $position_id){
        ...
        //机率pv率如果为空全部记录
        $this->website->record_pv = config('app.recordpv.1000'); //默认值25%
        foreach(config('app.recordpv') as $key=>$val)
        {
        ...
        for($i = 0; $i < config('app.number'); $i++)
        {
            ...
            $rndadspv = mt_rand(1, config('app.rate'));		//前来源抽样概
        ...
        foreach(config('app.debug') as $key=>$val)
        {
            if($key==$this->website->userid)
            {
        ...
        if(strpos(server('http_referer'), 'debug=memcache'))
        {
            is_work_memcache($this->memcache_obj);
        }

        if($debug=='true') 
        {
            require __ROOT__.'/app/script/'.config('app.debug.'.$this->website->userid);
        }
        else if($debug=='test')
        {
    ...
    {
        //机率pv率如果为空全部记录
        $border = config('app.border.1000'); //默认值25%
        foreach(config('app.border') as $key=>$val)
        {
            if($key==$this->website->userid)
            {
                $border = $val;
