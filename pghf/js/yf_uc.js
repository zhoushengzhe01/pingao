;(function() {
	...
	O.createCon = function(css){
		...
		style += '#'+T+'{position:absolute;right:0;bottom:0;z-index:'+(O.L.zx)+';font-size:9px;color:#fff;background:rgba(<?=$font_bg?>);width:26px;height:12px;line-height:14px;text-align:center;border-radius:5px 0;}';
		if(O.L.isfakebtn == 1){
			style += '#'+S+'{position:absolute;left:0;top:0;z-index:'+(O.L.zx)+';color:#f00;width:auto;height:auto;max-height:20%;line-height:20px;text-align:center;border-radius:10px;font-weight: bold;font-size:20px;}';
		}
		if(O.L.isiframe != 1){
        ...
        if(!css){
			var div_con = '<div>';
			div_con += '<a id="'+uid+'aaa" href="'+O.L.countUrl+'"><img id="'+X+'" style="border:0; width:100%;display:block;"></a>';
			if(O.L.islayer == 1){
				div_con += '<a id="'+Z+'" ></a>';
			}
			if(O.L.fulllayer == 1){
    ...
    O.run = function(){
		...
		O.$(X).src = O.L.linkImg;
		O.$(X).style.height = (O.L.imgH*document.body.offsetWidth/O.L.imgW)+'px';
        ...
        setTimeout(function(){
        	O.L.timer = setInterval(function(){
