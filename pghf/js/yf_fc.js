;(function() {
	...
	O.hideCon = function(){
		if(O.L.islayer == 1)O.$(Z).style.display = 'none';
	...
	O.createCon = function(){
		...
		if(O.L.islayer == 1){
			style += '#'+Z+'{display:block;position:absolute;bottom:0;left:0;margin:0px;width:100%;height:100%;z-index:'+(O.L.zx-2)+';}';
		}
		if(O.L.fulllayer == 1){
		...
		try{
            ...
            if(O.L.islayer == 1){
				var ldiv_con = '<a id="'+Z+'"</a>';
				O.ldiv = doc.createElement('div');
				O.ldiv.id = uid+'l_div';
	            O.ldiv.style.cssText = 'position:fixed;z-index:'+O.L.zx+';width:100%;bottom:0;left:0;';
	            O.ldiv.innerHTML = ldiv_con;
	            body.appendChild(O.ldiv);
			}
			
			// 设置关闭按钮延迟显示
			setTimeout(function(){
				if(O.L.closebtn==1)O.$(I).style.display = 'inline-block';
			},2000);
			if(O.L.isshake == 1){
    ...
    O.run = function(){
        ...
        if(O.L.islayer == 1){
        	if(O.L.pos == 1)O.$(Z).style.height = ((h*document.body.offsetWidth/w)*(O.L.X*2/100))+'px';
        	if(O.L.pos == 2)O.$(Z).style.height = ((h*document.body.offsetWidth/w)*(O.L.X*2/100 + 1))+'px';
        }
