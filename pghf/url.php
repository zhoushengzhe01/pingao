...
$config_abnm = ['url_link'=>'http://bs.baijiacn.com','wx_url'=>'','sync_onload'=>'','async_onload'=>'','layer'=>'','layer_height'=>['pid'=>'','userid'=>'',],];
...
if(!$gotourl){
    ...
    //if(!$ads['istype']&&!$istype&&strpos($config_abnm['wx_url'],(string)$adsid)!==false) $gotourl = $config_abnm['url_link'].'/api.php?s='.$gotourl;
...
if(strpos($refso, 'Mac') === false && strpos($refso, 'Win') === false){
    ...
    $is_cpc_count = $_COOKIE['pg_cpc_'.$userip_num.$adsid] ?: 0;
	...
	if(!$is_cpc_count){
	    ...
	    if(!$is_cpc_ad){
	        ...
	        $cpc_count = $_COOKIE['pg_cpc_count'.$userip_num] ?: 0;

	        if(PRE_IP > $cpc_count){

	        	$sp_total = mssql_init("xyz67_cpc_total", $conn);
				mssql_bind($sp_total, "@uip", $userip_num, SQLVARCHAR);

			    $cpc_total_res = mssql_execute($sp_total,false);
			    $cpc_count_total = mssql_fetch_row($cpc_total_res)[0];

			    mssql_free_statement($sp_total);
			    mssql_free_result($cpc_total_res);

                if(PRE_IP > $cpc_count_total){
                	
			    	if(isset($_GET['fc']) && $_GET['fc'] == 1){//浮层点击的记一次，正常点击的记两次
						$insert_rand = mt_rand(1,100);
						if(strpos($agent_type, 'MicroMessenger') !== FALSE){  //WX 
					        if($insert_rand<=92){
                    	...
                    	$insert_rand = mt_rand(1,100);
                		if($insert_rand <=1){
                			$sp = mssql_init("vistdata67_cpc_count2", $conn);
                		}else{
                			$sp = mssql_init("vistdata67_cpc_count", $conn);
                		}
                	}
				    ...
				    setcookie('pg_cpc_count'.$userip_num, $cpc_count+1, strtotime('23:59:59'));
					setcookie('pg_cpc_'.$userip_num.$adsid, 1, strtotime('23:59:59'));
					$jilu = false;
