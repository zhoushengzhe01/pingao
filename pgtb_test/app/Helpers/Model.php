...
class Model extends Mssql
{
    ...
    public function getAdposition($pid, $tid, $memcache)
    {
        ...
        {            
            ...
            {
                //进行缓存
                $memcache->set($cacheKey, $object, MEMCACHE_COMPRESSED, AppConfig::get('memcache.time'));
            }
            else
            {
                $memcache->set($cacheKey, 'null', MEMCACHE_COMPRESSED, AppConfig::get('memcache.time'));
                return false;
            }
    ...
    public function getWebsiteUrl($websiteId)
    {
        ...
        if(empty($string))
        {
            ...
            if($object)
            {
                ...
                //进行memcache缓存
                //$memcache->set($cacheKey, $string, MEMCACHE_COMPRESSED, AppConfig::get('memcache.time'));
    ...
    public function getAds($tid, $memcache, $update = false)
    {
        ...
        if(!$memcache->get('pgtb_adslist_isava'.$tid) || $update)
        {
            ...
            $memcache->set('pgtb_adsinfo', $ads->list, MEMCACHE_COMPRESSED, AppConfig::get('memcache.time'));
            $memcache->set('pgtb_adslist_'.$tid, $ads->info, MEMCACHE_COMPRESSED, AppConfig::get('memcache.time'));
    ...
    public function uptype($tid, $memcache, $update = false){
        ...
        {
            $objects = $this->select($sql);
            $memcache->set(md5($sql), $objects, MEMCACHE_COMPRESSED, AppConfig::get('memcache.time'));
