...
class Mssql
{
    ...
    public function __construct($name='mssql')
    {
        ...
        $this->host = AppConfig::get($name.'.host');    //连接地址

        $this->port = AppConfig::get($name.'.port');    //端口

        $this->database = AppConfig::get($name.'.database');    //数据库

        $this->username = AppConfig::get($name.'.username');    //连接用户名

        $this->password = AppConfig::get($name.'.password');    //连接密码

        $this->isP = AppConfig::get($name.'.isP');;    //是否常
