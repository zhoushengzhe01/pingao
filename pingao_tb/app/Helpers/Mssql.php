...
class Mssql
{
    ...
    public function __construct($name='mssql')
    {
        ...
        $this->host = config('database.'.$name.'.host');    //连接地址

        $this->port = config('database.'.$name.'.port');    //端口

        $this->database = config('database.'.$name.'.database');    //数据库

        $this->username = config('database.'.$name.'.username');    //连接用户名

        $this->password = config('database.'.$name.'.password');    //连接密码

        $this->isP = config('database.'.$name.'.isP');;    //是否常
