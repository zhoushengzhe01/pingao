;(function() {
	...
	O.L = {
		...
		zx: 10000,
		closePic: "<?=config('app.pic_url')?>afa.png",	// 关闭按钮图片
		fakedClosePic: "<?=config('app.pic_url')?>Close2nn.png",	// 假关闭按钮
		isiframe: "<?=$config['isiframe']?>",	// iframe不要横竖屏判断
		sh: "<?=$config['sh']?>",
		pos: "<?=$this->ad_pos?>",	//位置 1左边 2右边
	...
	var icon_ads = 'icon_ads-'+uid,
		icon = 'icon-'+uid,
		close = 'close-'+uid,
		advertise = 'advertise-'+uid,
		images = 'images-'+uid,
		image = 'image-'+uid,
		action_start = 'action_start-'+uid,
		action_stop = 'action_stop-'+uid;
	...
	O.createCon = function(){
		...
		var style = `
			...
			#`+icon_ads+` #`+images+` .`+action_start+`{
				transform:rotateY(90deg);
			}

			#`+icon_ads+` #`+images+` .`+action_stop+`{
		...
		// if(O.L.closebtn == 1){
		// 	style += '#'+I+'{position:absolute;'+(O.L.pos == 1?'bottom:':'top:')+'-18px;right:0;z-index:'+O.L.zx+';width:20px;height:18px;background:rgba(0,0,0,0.1);font-size:15px;text-align:center;line-height:19px;color:#fff;font-family: Arial;text-decoration:none;}';
		// }else if(O.L.closebtn == 2){
		// 	style += '#'+I+'{position:absolute;'+(O.L.pos == 1?'bottom:':'top:')+'-18px;right:0;z-index:'+O.L.zx+';width:20px;height:18px;background:rgba(0,0,0,0.1);font-size:15px;text-align:center;line-height:19px;color:#fff;font-family: Arial;text-decoration:none;}';
		// }
	
		// style += '#'+T+'{position:absolute;right:0;bottom:0;z-index:'+(O.L.zx)+';font-size:9px;color:#fff;background:rgba(57,146,227,0.4);display:inline-block;width:26px;height:12px;line-height:14px;text-align:center;border-radius:5px 0;}';
		// if(O.L.isfakebtn == 1){
		...
		var div_con = `
			<div id='`+icon_ads+`'>
				<div id='`+icon+`'>
					<span id="`+close+`">✕</span>
					<!--<span id="`+advertise+`">广告</span>-->
				</div>
				<div id="`+images+`">
					<a onclick="`+uid+`.linkgo(7)"><img id="`+image+`" src='`+O.L.imgsrc[0]+`'/></a>
				</div>
			</div>
		`;
		...
		try{
			...
			// 设置关闭按钮延迟显示
			setTimeout(function(){
				O.$(icon).style.display = 'inline-block';
			},2000);
			//if(O.L.isshake == 1){
	...
	// O.change = function(key)
	// {
	// 	if( key>=O.L.imgsrc.length-1 )
	// 	{
	// 		key=0;
	// 	}
	// 	else
	...
	// 	console.log(key);
	// 	console.log(O.L.imgsrc);

	// 	var div = O.$(image);
	// 	div.className=action_start;

	// 	setTimeout(function(){
	// 		div.setAttribute('src', O.L.imgsrc[key]);
	// 		div.className=action_stop;
	// 	}, 1000);

	// 	console.log('okok');
	// 	//setTimeout(O.change(key), 5e3);
	// }
	...
	O.changeImage = function(key){
		...
		setTimeout(function(){
			setInterval(function(){
				...
				// O.$(image).src = O.L.imgsrc[O.L.i];
				// O.L.countUrl = O.L.urllinks[O.L.i];
				

				O.$(image).className=action_start;

				setTimeout(function(){
					O.$(image).src = O.L.imgsrc[O.L.i];
					O.$(image).className=action_stop;
				}, 1000);
	...
	<?php if($onload){ ?> O.ready(function(){ <?php } ?>
		...
		O.loadImg(O.L.linkImg,function(){
			...
			O.$(icon_ads).style.height = (h*document.body.offsetWidth/w)+'px';

			//O.$(image).src = O.L.linkImg;
			//O.$(image).style.height = (h*document.body.offsetWidth/w)+'px';
